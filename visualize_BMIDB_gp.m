classdef visualize_BMIDB_gp < visualize_BMIDB
    methods (Static, Access = public)
        ...
        function Topo(self,config_preproc)
            taskwin = 1+config_preproc.time_win(1)*config_preproc.fs_fft : 1+sum(config_preproc.time_win(1:2))*config_preproc.fs_fft;
            vi = visualize_BMIDB_gp;
    ...
    methods (Static, Access = protected)
        function ERSP = gp_ERSP_(self,coi)
            ...
            for i_ses = 1 : num_ses
                ...
                data_ses = nanmean(data_ses,numel(size(data_ses)));
                ERSP{i_ses} = data_ses;
            end
        end
        ...
        function ERSP = ERSP_(ERSP_all,config_preproc)            
            ...
            ERSP = [];
            for i_sub = 1 : num_sub
                mean_sub = nanmedian(ERSP_all{i_sub},3);                
                if i_sub == 1 
                    ERSP = mean_sub;
                else
                    ERSP = Atom.dyncat(3,ERSP,mean_sub);
                end                
            end            
        ...
        function Topo_(Topo_all,config_preproc)
            ...
            ERSP = [];
            for i_sub = 1 : num_sub
                mean_sub = nanmedian(Topo_all{i_sub},2);                
                if i_sub == 1 
                    ERSP = mean_sub;
                else
                    ERSP = cat(2,ERSP,mean_sub);
                end  
            end
